...
- **Closed type:** `type Decision = Replay(ReplayScope) | AbortReplay` where `ReplayScope` is closed
- **Sink / capability:** `grant replay irreversible` → `commit { replay(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/replay-gate/demo.sh
```
...
- `replay-gate_safe.wave` — the correct design.
- `replay-gate_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
